...
versions() {
  ...
  ROOK_CEPH_VERSION=${ROOK_CEPH_VERSION:-v1.19.3}
  LOCAL_PATH_VERSION=${LOCAL_PATH_VERSION:-v0.0.35}
  OPENTELEMETRY_OPERATOR_VERSION=${OPENTELEMETRY_OPERATOR_VERSION:-0.109.0}
  OPENTELEMETRY_COLLECTOR_VERSION=${OPENTELEMETRY_COLLECTOR_VERSION:-0.147.1}
  ...
  SHARED_GATEWAY_VERSION=${SHARED_GATEWAY_VERSION:-v1.5.1}
  CILIUM_VERSION=${CILIUM_VERSION:-1.19.2}
  EXTERNAL_SECRETS_VERSION=${EXTERNAL_SECRETS_VERSION:-2.2.0}
}
...
shared-gateway() {
  fetch_url \
    "shared-gateway" \
    "${SHARED_GATEWAY_VERSION}" \
    "https://github.com/kubernetes-sigs/gateway-api/releases/download/${SHARED_GATEWAY_VERSION}/experimental-install.yaml" \
    "shared-gateway" \
    "true" \
    "true" \
    "" \
...
cephClusterSpec:
  annotations:
    all:
      kubernetes.io/egress-bandwidth: "60M"
      kubernetes.io/ingress-bandwidth: "60M"
cephBlockPools:
- name: ceph-blockpool
  spec:
    failureDomain: host
