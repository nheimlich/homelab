...
CUR_K8S=$(kubectl get --raw /version 2>/dev/null | sed -nE 's/.*"gitVersion": "v?([0-9.]+).*/\1/p' | head -n1)
...
check_upgrade "Talos" "${CUR_TALOS}" "${UP_TALOS}" \
  "talosctl upgrade -i factory.talos.dev/metal-installer-secureboot/${FACTORY_ID}:v${UP_TALOS} --context ${CTX} --wait=false --drain=false"
